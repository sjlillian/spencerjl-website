...
function formatRepoName(name) {
  ...
  return name
    .split(/[-_]+/)
    .map(word => {
      if (/^(mtg|ai|dsd|vr|be|fe)$/i.test(word)) {
...
export async function getProjectsData() {
  try {
    ...
    let res;
    res = await fetch(`https://api.github.com/users/${owner}/repos?per_page=100`, {
      headers: { 'User-Agent': 'spencerjl-website' },
    ...
    return portfolioRepos.map(repo => {
      const stack = repo.topics
        ...
        .map(topic => {
          if (/^(mtg|ai|dsd|vr|be|fe)$/i.test(topic)) {
...
export async function getBlogBySlug(slug) {
  try {
    ...
    // Extract title: use frontmatter title if exists, otherwise slug (filename)
    const title = matterResult.data.title || decodedSlug;
